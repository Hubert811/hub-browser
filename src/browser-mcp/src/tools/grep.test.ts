...
function sessionWithSnapshot(text: string): unknown {
  return {
    observe: () => ({
...
function pageWithSnapshot(text: string) {
  return pageFromSession(sessionWithSnapshot(text))
}
...
describe('grep tool', () => {
  ...
  it('reports zero matches with an empty matches array', async () => {
    const result = await executeTool(
      ...
      makeContext(pageWithSnapshot('link "Home"\nlink "About"')),
    )
  ...
  it('preserves refs while clamping matches and spills the full line', async () => {
    await withBrowserosDir(async () => {
      ...
      const result = await executeTool(
        ...
        makeContext(pageWithSnapshot(line)),
      )
  ...
  it('clamps requested limits to the shared maximum', async () => {
    ...
    const result = await executeTool(
      ...
      makeContext(pageWithSnapshot(haystack)),
    )
