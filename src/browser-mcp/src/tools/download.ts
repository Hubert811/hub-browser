...
export const download = defineTool({
  ...
  handler: async (args, ctx) => {
    ...
    const { suggestedFilename } = await captureDownload(page, dir, args.ref)
...
/**
 * Arms page download capture, runs the trigger, and resolves when the
 * download completes — mirroring the vendored browser-mcp implementation:
 * subscribe Page.downloadWillBegin/downloadProgress on the live page session
 * instead of polling the download directory.
 */
async function captureDownload(
  ...
  const session: ProtocolApi = await page.pageSession()
