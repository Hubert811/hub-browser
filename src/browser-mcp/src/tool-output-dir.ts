...
/**
 * User data root (方案 C): `~/.hub` by default, overridable via BROWSEROS_DIR
 * (trimmed; empty/whitespace falls back to the default). Kept in sync with the
 * engine fork's hubUserRoot() so MCP tool outputs land under the same root.
 */
export function getBrowserosDir(): string {
  const override = process.env.BROWSEROS_DIR?.trim()
  if (override) return override
  return join(homedir(), '.hub')
}
