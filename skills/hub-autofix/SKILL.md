---
name: hub-autofix
description: 当 `hub <site> <command>` 因站点改了 DOM、API 或响应结构而失败时，自动诊断、修 adapter 并重试，而不是只报错。采集 trace artifact → 改 adapter → 重试 → 修复验证后上报上游 issue。适用于任意 AI agent。
allowed-tools: Bash(hub:*), Bash(gh:*), Read, Edit, Write
---

# hub-autofix — Automatic Adapter Self-Repair

When a `hub` command fails because a website changed its DOM, API, or response schema, **automatically diagnose, fix the adapter, and retry** — don't just report the error.

## Safety Boundaries

**Before starting any repair, check these hard stops:**

- **`AUTH_REQUIRED`** (exit code 77) — **STOP.** Do not modify code. Tell the user to log into the site in the browser session (hub 共享用户登录态)。
- **`BROWSER_CONNECT`** (exit code 69) — **STOP.** Do not modify code. hub 无 doctor；让用户确认浏览器在跑、`BROWSEROS_CDP_PORT` 指向正确端口，然后重跑。
- **CAPTCHA / rate limiting** — **STOP.** Not an adapter issue.

**Scope constraint:**
- **Only modify the file at `adapterSourcePath` in the trace `summary.md` front matter** — this is the authoritative adapter location (may be `clis/<site>/` in repo or `~/.hub/clis/<site>/`)
- 官方自带 adapter 要改时，先 `hub adapter eject <site>` 拷一份到 `~/.hub/clis/` 再改——**不要直接改 repo `clis/` 里的官方文件**，也不要改 `src/`、`tests/`、`package.json`、`tsconfig.json`
- **Never modify** `src/`, `tests/`, `package.json`, or `tsconfig.json`
...
```bash
hub browser <s> state   # 连通自检：能返回 URL/title 即可（hub 无 doctor；BROWSEROS_CDP_PORT 可达性由浏览器决定）
```

## When to Use This Skill

Use when `hub <site> <command>` fails with repairable errors:
...
- **Retry with an alternative query or entry point.** If `hub xiaohongshu search "X"` returns 0 but `hub xiaohongshu search "X 攻略"` returns 20, the adapter is fine — the platform was shaping results for the first query.
- **Spot-check in a normal browser tab.** If the data is visible in the user's own browser but the adapter comes back empty, the issue is usually authentication state, rate limiting, or a soft block — not a code bug. The fix is re-login in the browser session, not editing source.
...
```bash
hub <site> <command> [args...] --trace retain-on-failure 2>trace-error.yaml
```
...
trace:
  traceId: "..."
  dir: "/path/to/.hub/profiles/default/traces/..."
  summaryPath: "/path/to/.hub/profiles/default/traces/.../summary.md"
  receiptPath: "/path/to/.hub/profiles/default/traces/.../receipt.json"
  status: failure
```
...
Use `hub browser <session>` to inspect the live website. **Never use the broken adapter** — it will just fail again.

### DOM changed (SELECTOR errors)

```bash
# Open the page and inspect current DOM
hub browser <s> open https://example.com/target-page && hub browser <s> state
...
```bash
# Open page, then trigger the action manually
hub browser <s> open https://example.com/target-page && hub browser <s> state

# Interact to trigger API calls
hub browser <s> click <N> && hub browser <s> network

# Narrow to the request you care about by the fields its body should have
hub browser <s> network --filter author,text,likes

# Inspect specific API response (key is the `key` field from the default JSON output)
hub browser <s> network --detail <key>
```

## Step 4: Patch the Adapter

Read the adapter source file at `adapterSourcePath` from the trace summary front matter and make targeted fixes. This path is authoritative — it may be in the repo (`clis/`) or user-local (`~/.hub/clis/`).
...
If the retry **passes**, the local adapter has drifted from upstream. File a GitHub issue so the fix flows back to `Hubert811/hub-browser`.
...
```markdown
## Summary
hub autofix repaired this adapter locally, and the retry passed.

## Adapter
- Site: `<site>`
- Command: `<command>`
- hub version: `<version from hub --version>`
...
_Issue filed by hub autofix after a verified local repair._
```
...
```bash
gh issue create --repo Hubert811/hub-browser \
...
- **Major redesign** — needs full adapter rewrite via `hub-adapter-author` skill
...
```
1. User runs: hub zhihu hot
   → Fails: SELECTOR "Could not find element: .HotList-item"

2. AI runs: hub zhihu hot --trace retain-on-failure 2>trace-error.yaml
...
4. AI explores: hub browser s open https://www.zhihu.com/hot && hub browser s state
...
6. AI verifies: hub zhihu hot
   → Success: returns hot topics

7. AI prepares upstream issue draft, shows it to the user

8. User approves → AI runs: gh issue create --repo Hubert811/hub-browser --title "[autofix] zhihu/hot: SELECTOR" --body "..."
```
