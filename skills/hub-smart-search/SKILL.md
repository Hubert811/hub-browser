---
name: hub-smart-search
description: 基于 hub 命令的智能搜索路由器。当用户想要使用 hub、CLI 或 API 搜索、查询、查找或研究信息时，尤其是涉及指定网站、社交媒体、技术资料、新闻、购物、旅游、求职、金融或中文内容时，务必使用此 skill。
allowed-tools: Bash(hub:*), Read, Grep
---

# 智能搜索路由器

根据话题和场景，将查询路由到最佳的 hub 搜索源。此 skill 的核心目标不是记忆命令，而是先定位数据源，再让 Agent 通过 `hub` 自己读取实时帮助，避免文档漂移。
...
- 运行 `hub <site> -h` 查看该站点有哪些子命令
- 若已锁定某个子命令，再运行 `hub <site> <command> -h` 查看参数、输出列、策略

不要在 skill 文档里硬编码参数或假设命令签名；以 `hub ... -h` 的实时输出为准。
...
- `hub list -f yaml`、`hub <site> -h`、`hub <site> <command> -h` 属于预检与帮助，不计入搜索次数
- 一次真正的 `hub <site> ...` 搜索/查询执行，计为该站点 1 次调用
...
- 始终以 `hub list -f yaml` 与 `hub <site> -h` 的实际结果为准
