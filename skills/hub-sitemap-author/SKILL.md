---
name: hub-sitemap-author
description: 当需要为站点创建或维护 hub 的 sitemap（面向 agent 的导航、页面状态、action、workflow、API 引用、pitfall 与 fallback 知识）时使用。浏览器探索发现了持久的站点上下文、sitemap 已过期、或要把本地站点知识提升进 repo 时触发。
allowed-tools: Bash(hub:*), Read, Edit, Write, Grep
---

# hub-sitemap-author

You are authoring a **task execution graph for agents**, not an SEO sitemap. The artifact should help an agent using `hub browser <session>` decide where it is, what path to take next, which hub adapter to prefer, and how to recover when the page disagrees with memory.
...
- **Global seed**: `sitemaps/<site>/` (repo top-level, 随 hub-browser repo 提交)
- **Local overlay**: `~/.hub/config/sites/<site>/sitemap/`
...
2. **Verify reality**: use `hub browser <session> state`, `find`, `network`, and `analyze`; browser state is truth. If you just completed a `hub-adapter-author` session for this site, start from the retained browse trace under `~/.hub/profiles/<context>/traces/` as seed evidence instead of re-discovering the path from zero.
...
5. **Write local draft**: update `~/.hub/config/sites/<site>/sitemap/...` unless explicitly promoting to repo.
...
evidence: hub browser <cmd> or trace:<path>
```
...
Do not promote an action without evidence. If a recovery path marks `adapter_health_update`, the sitemap consumer (hub-browser-sitemap) must write that health update to the local overlay so the next agent does not retry a known-suspect adapter.
...
- **Best path**: prefer existing `hub <site> <command>` adapter if it covers the goal.
...
```yaml
on_adapter_fail:
  - adapter_health_update: hub twitter post -> suspect
  - hub browser state (verify current page)
...
- Drafts go inside `sitemap/draft-<topic>.md`, not `~/.hub/config/sites/<site>/sitemap.draft.md` at the parent level — the latter is invisible to `hub browser` sitemap availability detection.
