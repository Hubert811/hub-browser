...
12 patches，按主题分 3 组。验证基础：twitter PoC（12 files / hub-user）+ hackernews PoC（10 files / hub-质量官）双站。
...
`auth_strategy` 取值：`PUBLIC_API | COOKIE_API | PAGE_FETCH | INTERCEPT | DOM_STATE | UI_SELECTOR`，定义见 [`../../hub-adapter-author/references/strategy-selection.md`](../../hub-adapter-author/references/strategy-selection.md)。
...
`Linked APIs` 引用的 `endpoint_id` 必须存在于 `apis.md`（间接也在 `endpoints.json`）。如果该站 `endpoints.json` 还在收集中（如 twitter 32+ GraphQL operation 大部分没登记），page `Linked APIs` 留空合理，**不要塞 fake id 占位**。后续 hub-adapter-author 补 `endpoints.json` 后再回填。
...
**Required per entry**:
- `endpoint_id` — 必须存在于同站 `~/.hub/config/sites/<site>/endpoints.json`
...
`pitfalls.md` 只放 **task-executor 级**坑 — 跑命令 / 操作页面会撞到的坑。**adapter-internal 实现坑**（queryId 解析 / envelope unwrap / bigint id / 字段 silent rename）放在 `~/.hub/config/sites/<site>/notes.md`，不进 sitemap。
...
Evidence:
- observed_with: hub browser <session> <command>
...
| `\|\|` | 多 do path / recovery path **fallback priority**（"前者失败试后者"）| `do: hub twitter like <url> \|\| click [data-testid="like"]` |
| `;` | 多 recovery 指令 **sequential**（"逐条执行"）| `recover: adapter_health_update: hub twitter like -> suspect; dom_click within card scope` |
...
**Do**：实际操作。优先级：
1. 已有 adapter 命令（`hub twitter post`）
2. semantic browser command（`hub browser <s> click "Post" button`）
...
- 或回 探测模式（"re-run `hub browser <s> find` for 'Post' button with a11y role=button"）
...
do: hub twitter like <tweet-url> || click [data-testid="like"] (within card scope)
post: testid 翻转 like -> unlike，icon 红色
fail: testid 不变 | 弹 login modal
recover: adapter_health_update: hub twitter like -> suspect; dom_click within card scope
evidence: hub twitter like + hub browser click
```
...
语义：当**本 action 因为该 adapter 失败而触发 Recovery**时，consumption skill（`hub-browser-sitemap`）必须：
...
**实现责任**：在 `hub-browser-sitemap` skill 的 consumption loop 里。Schema 这里只 spec directive 写侧格式，不 spec 实现细节。

**Recovery 回 `healthy` 不在本 schema 范围**：`adapter_health` 从 `suspect` 回 `healthy` 的路径（TTL 自动衰减 / 跑 Fallback 成功后 probe Best path / 人工 reset）留给 `hub-browser-sitemap` skill spec 拍。本 PR 只定写侧（"failed → suspect"），不定读侧（"suspect → healthy"）的恢复策略。
...
**❌ 不要**放在父目录（如 `~/.hub/config/sites/<site>/sitemap.draft.md`） — `hub browser <s> open` 的 sitemap availability 检测只看 `sitemap/` 目录是否存在（含 repo `sitemaps/<site>/`）。draft 放父目录 → 检测不到 → agent 不会被提醒"有 sitemap" → 你的发现没人用。

正确：
```
~/.hub/config/sites/twitter/sitemap/
├── SITE.md
├── pages/home.md
...
`hub browser <s> open` 用 adapter registry 把 hostname → site 映射（如 `news.ycombinator.com → hackernews`）。如果 sitemap 先于 adapter 存在（即一个站还没人写 adapter 但有人写了 sitemap），registry 没数据，sitemap dir 检测不到。
...
2. agent 必须现场探测（用 `hub browser <s> state` / `find`）
...
正确：
- ✅ "sitemap 说有 Post 按钮 → `hub browser <s> find` 一下 → 不在 → 标 stale + 探测真实页面有没有别的发帖入口"
...
- action `Postconditions` 里的 `url_pattern` / `dom_anchor` → 用 `hub browser <s>` 实跑一遍，验证 anchor 仍可 resolve
