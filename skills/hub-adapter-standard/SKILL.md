...
2. **逐组件攻克**：一个组件一个组件地走「侦察 → 手测 → 锚点链 → 回写 → 单组件验证」，通过后才进下一个——**禁止一次性写完所有组件再跑测试矩阵**（失败时无法定位）。两条配套纪律：**验证通道必须与实现通道一致**（手测用什么通道，适配器就用同一通道实现）；**不在污染页面上叠试错**（每次验证从已知干净状态出发，页面异常唯一正确动作是页面级复位）。迭代语义：**改适配器/插件文件（含 lib/ 链）无需重启 daemon 或 MCP server**——下一条命令/工具调用前自动整树重载（clis 与 plugins 是同一重载单元，任一改动两树都重求值；镜像在 `~/.hub/.adapter-reload/` 与 `~/.hub/.plugin-reload/`，模块级状态随之重置，适配器状态须可丢弃）；需要杀 daemon 的只有引擎/CLI 自身代码改动
