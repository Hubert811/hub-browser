...
🟡 的维度意思：方法在文档里，但这一版没拿真实站点跑过端到端。第一次遇到时按文档走，遇到和文档不一致的地方记到 `~/.hub/config/sites/<site>/notes.md`，下一次再打开就是 ✅。
...
| 类型 | 证据 adapter | 覆盖维度 |
|------|-------------|---------|
| PUBLIC + 自解释字段 + SPA | `~/.hub/clis/coingecko/top.js`（本轮 dry run） | `Strategy.PUBLIC` + REST JSON + 自解释字段 + 列表页 |
| COOKIE + 代号字段 + JSONP | `clis/eastmoney/*.js` × 13（已 merged） | `Strategy.PUBLIC`（匿名行情）+ 代号字段 + JSONP/CSV + 列表页 |
| UI_SELECTOR + 写操作 | `clis/chatgpt/*.js`（agent app） | 表单 + 多语言 fallback selector |
| INTERCEPT + 签名 | `clis/xiaohongshu/*.js`（signed URL） | `a_bogus` signature + INTERCEPT |
| DOM_STATE | `clis/bilibili/*.js` 部分 | hydration / inline JSON 兜底 |

---

## 反模式：为过 verify 而放松 fixture

**verify 绿 ≠ 数据对**。fixture 的 `patterns` / `notEmpty` / `mustNotContain` / `mustBeTruthy` 失败 = adapter 输出坏了，**收紧 adapter，不是放松 fixture**。唯一合法改 fixture 的场景是站点本身换格式（URL 规范迁移等），且要在 `~/.hub/config/sites/<site>/notes.md` 顶部说明。详见 `success-rate-pitfalls.md` §1 和 `typed-errors.md` §6。
