...
hub 用 5 类 typed error 让 agent 能从 exit code 直接分辨"参数错 / 没数据 / 接口挂 / 要登录 / 超时"。silent `return []` / silent `return [{sentinel}]` / scalar sentinel (`'-'`) / `Math.max/min` silent clamp / generic `CliError('HTTP_ERROR')` 这些"绿但错"的写法都被 `hub convention-audit` 抓得越来越紧（baseline 只能减不能加，新违例必须立刻收掉）。

> 代码里的 import 一律沿用 `hub browser init` 骨架自带的 registry / errors 两个子路径（hub 运行时提供）。不要手改包名、不要加第三方依赖。下文代码块里的 `ArgumentError` 等类名即来自骨架 errors 导入。
...
**禁止泛用 `CliError('HTTP_ERROR' / 'NO_DATA' / 'USER_NOT_FOUND' / 'FETCH_ERROR' / 'INVALID_ARGUMENT' / 'API_ERROR' / 'THREAD_NOT_FOUND')`** —— 这些都对应到 5 类里其中一个，没有不能映射的场景。`CliError` 基类只用作子类的实现细节，adapter 代码不应直接 new。
...
**修法**（同站 `clis/<site>/utils.js` 的 `normalizePositiveInteger` / `normalizeLimit`）：
...
**注意**：`hub convention-audit` 的正则只抓 `Math.min(...limit...)`。`Math.max(1, ...)` 单边 floor 和非-`limit` 命名（`page` / `timeout` / `contentLimit`）都不在 regex 里，所以必须靠**人审 checklist** 兜：**任何外部参数（args.\* / kwargs.\*）做 Math.max / Math.min / `|| N` 当兜底都是 silent-clamp**。
...
if (items.length === 0) {
    throw new EmptyResultError('search', `No results for "${query}"`);
}
if (!result.ok && !result.sessions.length) {
    throw new CommandExecutionError(`history API failed (status=${result.status})`);
}
if (!asset?.ok) {
...
1. **round-trip pipeline**：listing → detail 类下游 adapter 拿到 `tid: ''` 会去查空串，白跑一轮
...
if (!/id="postlist"/.test(html)) throw new EmptyResultError('thread', `帖子 ${tid} 不存在`);
```
...
新写 fixture 时（`~/.hub/config/sites/<site>/verify/<cmd>.json`）：
...
repo 里仍有相当数量的 `CliError('HTTP_ERROR')` / `Math.max(1, Math.min(...))` 式旧写法，被 `hub convention-audit` 的 baseline 圈住（baseline 只允许减、不允许加）。**新写 adapter 必须按本文档**；旧 adapter 不强制立刻迁移，但碰到时顺手收一条是欢迎的——清掉一条 baseline 自然下降一条，gate 不会卡。
...
- [ ] adapter 抛的不是 generic `CliError('XXX')`，而是 5 类 typed error 之一
- [ ] verify fixture 的 `rowCount.min ≥ 1`，sentinel row 过不了
- [ ] `hub convention-audit --strict` 跑过 → baseline 没增加
