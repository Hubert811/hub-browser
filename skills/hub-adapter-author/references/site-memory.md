...
站点记忆分两层：**repo 种子**（skill 自带的已知站点公共知识）+ **本地用户层**（每台机器跑过的站点累积产物）。
...
```
skills/hub-adapter-author/references/site-memory/<site>.md
    — 公共种子。手写 + PR 审核进入。多 agent 共享的第一批起点。
    — 已铺：eastmoney / xueqiu / bilibili / tonghuashun

~/.hub/config/sites/<site>/
    — 本地累积。agent 跑 adapter 过程里自动写入，跨 session 复用。
    — 不进 git，不进 PR。
```

用法：开头先读本地，命中 **不跳写 adapter**，仍要跑 Step 5 endpoint 验证 + Step 7 字段抽查（memory 可能过期或站点换版）；没命中读 repo 种子；都没有走完整 recon。

---

## Layer 1 — Repo 种子（`references/site-memory/<site>.md`）
...
## Layer 2 — 本地用户层（`~/.hub/config/sites/<site>/`）
...
~/.hub/config/sites/<site>/
  ...
  verify/                — `hub browser verify` 期望值（值级校验锚点，每个 adapter 一份）
    <cmd>.json
...
每个 adapter 一份，`hub browser <s> verify <site>/<cmd>` 会自动读。**没有这份 = verify 只能证"能跑"，证不出数据对**——所以是必填产物。
...
```json
{
  ...
  "expect": {
    ...
    "patterns": {
      "url": "^https://www\\.example\\.com/bbs/thread-"
    },
    "notEmpty": ["title", "url"]
  }
}
...
- `args`：verify 调 adapter 时要带的参数。支持两种形态：
  - **对象**：`{ "limit": 3 }` → 展开成 `--limit 3`，named-flag adapter 用
  - **数组**：`["1234567", "--limit", "3"]` → 原样 append，positional 主语型 adapter（`<tid>` / `<url>` / `<query>`）必须用数组形态
...
调试过程里的临时 dump（`.dbg-*.html` / `raw-*.json` / `sample-*` / `trace-*.txt`）**只能**落在 `~/.hub/config/sites/<site>/fixtures/` 或系统 `/tmp/`。PR diff 会把 repo 根目录和 `clis/<site>/` 下的文件一起带走——别人 review 时看到一堆调试副产物会很烦。

---

## 没有 site-memory 时

新站点没对应 `.md`，也没本地目录 → 完整走 recon + discovery，跑完直接写 `~/.hub/config/sites/<site>/`，后面就有了。
