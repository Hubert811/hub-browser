...
> 代码里的 import 一律沿用 `hub browser <s> init` 骨架自带的 registry / errors 两个子路径（hub 运行时提供）。下面代码块里省略了 import 行，写 adapter 时直接保留 init 骨架的 import，不要手改、不要加第三方依赖。
...
> **注意**：下面这份 `convertible.js` 的 limit clamp 和 `CliError('HTTP_ERROR' / 'NO_DATA')` 是 grandfathered 写法（在 `hub convention-audit` 的 baseline 里）。结构布局（cli 声明 / args / columns / map）仍然是好范本，但 **error 处理 + limit 校验请按下文 §3 + [`typed-errors.md`](./typed-errors.md) 写**。新写 adapter 抄这个文件别连 `Math.max(1, Math.min(...))` 和 `CliError(...)` 一起抄过去。

```javascript
// eastmoney convertible — on-market convertible bond listing.
// (import 行保留 init 骨架的 registry / errors 子路径)
...
cli({
  ...
  func: async (args) => {
    ...
    const limit = normalizePositiveInteger(args.limit, 20, 'limit', { min: 1, max: 100 });
    ...
    if (!resp.ok) throw new CommandExecutionError(`convertible failed: HTTP ${resp.status}`);
    const data = await resp.json();
    const diff = Array.isArray(data?.data?.diff) ? data.data.diff : [];
    if (diff.length === 0) throw new EmptyResultError('eastmoney convertible', 'no convertible data');
...
cli({
  ...
  description: '...',         // 一句话，出现在 `hub list` 和 `hub <site> -h`
...
- 列名 camelCase，跟同站点其他 adapter 保持统一
- **中间解析对象 key 不能跟 columns 任一项重叠** —— 否则 `silent-column-drop` audit 会把它当 row 候选误判。`{pid, html, start}` 这类中间结构改成 `{postId, body, offset}`，最后在 push row 时再 destructure aliasing 回 column 命名。
...
**参数形态**（**踩过最多次的坑**：搞反签名后 `args` 实际是 debug flag，所有 `args.foo` 静默 undefined → fallback 到 default）：
...
**错误处理**：用 typed error 5-classification（参见 [`typed-errors.md`](./typed-errors.md)），**不要** generic `CliError('XXX', ...)` 直传，**不要** `return []` 了事，**不要** `return [{sentinel}]` 装一行业务数据冒充 empty。hub-autofix skill 靠 typed error 的 exit code（66 = empty / 77 = auth / 75 = timeout / 2 = argument / 1 = exec）决定要不要重试。
...
同一个发送按钮在英文浏览器是 `aria-label="Submit"`，在中文浏览器（语言设置切中文）变 `aria-label="提交"`，CSS 选择器 `button[aria-label="Submit"]` 在中文环境下直接 0 匹配，silent empty result。
...
为什么不在浏览器端固定 locale？因为 hub **不启动浏览器**——hub 连用户已经在跑的浏览器（CDP 直连），用户可能就是中文 UI / 中文资料检索需求。强制 en-US 会破坏用户的正当工作流。
...
if (/暂时没有提醒内容/.test(html)) {
    throw new EmptyResultError('notifications', '暂时没有提醒内容');
}
```
...
## Verify fixture（每个 adapter 配一份 `~/.hub/config/sites/<site>/verify/<name>.json`）

verify fixture 是"adapter 产出长什么样"的结构锚点。没有它，`hub browser <s> verify` 只能证"adapter 能跑完不抛"，证不出数据没错位。**必写**。
...
hub browser <s> verify 1point3acres/hot --write-fixture

# 2. 手改 ~/.hub/config/sites/1point3acres/verify/hot.json
#    - patterns: 加 URL / 日期 / ID 正则
#    - notEmpty: 加核心字段（title / author / url）
#    - rowCount: 收紧到业务合理区间

# 3. 再跑 verify，fixture 吃得动就 OK
hub browser <s> verify 1point3acres/hot
```
...
hub 1point3acres thread 1173710 --limit 2 --format json | head

# 2. 照着响应手写 ~/.hub/config/sites/1point3acres/verify/thread.json
#    （args 一定用数组: ["1173710", "--limit", "2"]）

# 3. 跑 verify 核对
hub browser <s> verify 1point3acres/thread
```
...
```
~/.hub/clis/<site>/<name>.js    # 私人
clis/<site>/<name>.js           # repo 贡献
```

**两者在 `cli({...})` 层面完全一样**。差别只在运行入口：

- 私人：写完立即可跑（`hub <site> <name>`）
- repo：走 `cli-manifest.json` 注册，`hub verify <site>/<name>` 可冒烟

先在 `~/.hub/clis/` 调通再拷贝到 `clis/`。改官方自带 adapter 时用 `hub adapter eject <site>` 先拷一份到 `~/.hub/clis/` 再改。
