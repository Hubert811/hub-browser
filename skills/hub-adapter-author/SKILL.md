---
name: hub-adapter-author
description: 当需要给一个新站点写 hub 适配器（adapter），或给已有站点新增命令时使用。覆盖从站点侦察、endpoint 发现、字段解码、adapter 编写到 `hub browser verify` 通过的完整流程（30 分钟闭环）。临时的浏览器驱动（不写 adapter）见 hub-browser-browser；全局导览见 hub-browser。
allowed-tools: Bash(hub:*), Read, Edit, Write, Grep
---

# hub-adapter-author

你是要给一个站点写 adapter 的 agent。这份 skill 目标：**从零到通过 `hub browser verify` 的 30 分钟内闭环**。

全程用现有工具：`hub browser <session> *` / `hub browser <session> init` / `hub browser <session> verify`。没有新命令。
...
> hub 直连 CDP，没有 doctor。环境自检 = 起一个 browser session 跑 `hub browser <s> state`（能出 URL/title 即连通），或检查 `BROWSEROS_CDP_PORT` 指向的浏览器端口可达。端口一般不用手动设（v0.1.1 起自动探测：`BROWSEROS_CDP_PORT` → BrowserOS neo 配置 `ports.cdp` → 9005，决策 D7）。`hub browser bind/unbind` 在 hub 中不可用（直接报错），不需要也不能用它绑定页面——session 本身就是页面容器。

---

## 前置：看你落在哪

先拿 `references/coverage-matrix.md` 快速自测。三个问题：
...
**先定 strategy，再写 adapter。** 每次进入 Step 3/4 后、写代码前，必须产出一段 strategy note。没有这段 note，不要开始写 `clis/<site>/<name>.js`（或 `~/.hub/clis/<site>/<name>.js`）。
...
| `PAGE_FETCH` | internal-unstable | 只能在页面上下文 `fetch` 才能复用 same-origin/session/runtime | `hub browser <s> eval fetch(...)` 非空；必须解释为什么避不开内部接口 |
...
START
  ...
  ▼
┌──────────────────────────┐
│ 连通自检（无 doctor）     │── no ──→ 检查 BROWSEROS_CDP_PORT / 浏览器是否在跑；
│ hub browser <s> state    │          起新 session 重试
└──────────────────────────┘
  │ yes
  ▼
┌────────────────────────────────────────────────────┐
│ 读站点记忆：                                        │
│   1. ~/.hub/config/sites/<site>/endpoints.json      │
│   2. ~/.hub/config/sites/<site>/notes.md            │
│   3. references/site-memory/<site>.md（如已铺）      │
└────────────────────────────────────────────────────┘
...
└──────────────────────────┘
  ...
  ▼
┌──────────────────────────┐
│ hub browser <s> init      │  生成 ~/.hub/clis/<site>/<name>.js 骨架
│ 复制最像的邻居 adapter    │
│ 改 name / URL / 映射三处  │
└──────────────────────────┘
  │
  ▼
┌──────────────────────────┐
│ hub browser <s> verify    │── 失败 ──→ hub-autofix skill，用 --trace retain-on-failure 回对应步骤
└──────────────────────────┘
  │ 成功
  ▼
┌──────────────────────────┐
...
└──────────────────────────┘
  ...
  ▼
┌──────────────────────────┐
│ 回写 ~/.hub/config/sites/│  endpoints / field-map / notes / verify / fixtures
└──────────────────────────┘
  │
  ▼
DONE
...
```
[ ] 1. 连通自检：`hub browser <s> state` 返回 URL/title（或检查 BROWSEROS_CDP_PORT 可达）
[ ] 2. 读站点记忆：
       [ ] ~/.hub/config/sites/<site>/endpoints.json 存在？里面有想要的 endpoint？
...
[ ] 3. 侦察（site-recon.md）：
       [ ] **首选**：`hub browser <s> analyze <url>` 一步拿 pattern + 反爬 + 最近 adapter + next step
...
[ ] 9. 写 adapter（adapter-template.md）：
       [ ] hub browser <s> init <site>/<name>
       [ ] 找同站点或同类型最像的 adapter，cp 过来
       [ ] 改 name / URL / 字段映射
[ ] 10. hub browser <s> verify <site>/<name>
...
[ ] 12. 回写站点记忆（**verify 通过 + 肉眼比对对得上之后**，schema 见 `references/site-memory.md`）：
        ...
        [ ] `verify/<cmd>.json`：**必填。** `hub browser verify` 的期望值（args / rowCount / columns / types / patterns / notEmpty），Step 10 已经让你生成了，这里只是 checklist
        [ ] `fixtures/<cmd>-<YYYYMMDDHHMM>.json`：存一份该 endpoint 的完整响应样本（去掉 cookie / token / 用户私有字段再存），给后续字段对比 / 离线 replay 用
        [ ] 调试过程中如果在 repo / adapter 目录 dump 过临时文件（`.dbg-*.html` / `raw-*.json` / 等），**在 commit 前清干净**——这些本来就该落在 `~/.hub/config/sites/<site>/fixtures/` 或 `/tmp/`
```
...
| Step 10 verify 失败 | `fltt` 漏了 / 字段映射错 | hub-autofix skill；复现命令加 `--trace retain-on-failure` |
...
| `references/site-memory.md` | 总览：repo 种子 + 本地 `~/.hub/config/sites/` 的两层结构 |
...
| `references/typed-errors.md` | 写 `func` 主体之前必读：5 类 typed error 落点表（ArgumentError / EmptyResultError / CommandExecutionError / AuthRequiredError / TimeoutError）+ 三大 silent anti-pattern（silent-clamp / sentinel-row / generic error）的反例修法 |

---

## 关键约定

- adapter 只引 `hub browser init` 骨架自带的 runtime 导入（registry / errors 两个子路径，由 hub 运行时提供），不用第三方
- `columns` 数组和 `func` 返回对象 keys 完全对齐（含顺序）
- **中间解析对象 key 不能跟 `columns` 任一项重叠**（否则 silent-column-drop audit 误判；改成专属命名 + push row 时 destructure aliasing）
- **`browser:` field 决定 func 签名**：`browser:false → (args)`，`browser:true → (page, args)`。搞反时 `args` 实际是 debug flag，所有外部参数 silent fallback 到 default
...
- 写私人 adapter 用 `~/.hub/clis/<site>/<name>.js`（写完即跑）；要提 PR 才 copy 到 repo `clis/<site>/<name>.js`
- 站点记忆每轮回写：没记忆 → 用 skill → 产生记忆 → 下次变 5 分钟
- **调试过程中的原始 dump / 抓包 / HTML 样本只能落在 `~/.hub/config/sites/<site>/fixtures/` 或 `/tmp/`。严禁在 repo 根目录、`clis/<site>/` 或当前工作目录留 `.dbg-*.html / raw-*.json / sample.*` 这类临时文件**（PR diff 会带上去，别人 review 时很烦）。
...
- 诊断类：连通自检（`hub browser <s> state` / BROWSEROS_CDP_PORT）→ 看 `notes.md` → 搜 hub-autofix skill
